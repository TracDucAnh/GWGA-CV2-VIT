...
"Distilling the Knowledge in a Neural Network") -- BASELINE de so sanh voi
phuong phap chinh (Gromov-Wasserstein Alignment cua BLL posteriors, xem
GWGA/GWGA_single_input_vit.py). File nay GIU NGUYEN toan bo kien truc, du
lieu, va he thong ve hinh (UMAP, loss landscape, ECE, Hessian-trace
sharpness, OOD AUROC) cua ban GWGA, CHI THAY THE thanh phan "structural
alignment loss" (Gromov-Wasserstein) bang cong thuc distillation kinh dien
cua Hinton 2015:

    L_KD = CE(y_true, student) + beta_KL * KL(q(W_student) || prior)
           + alpha * T^2 * KL( softmax(teacher_logits / T)
...
Day la phep thay the 1-doi-1 cho gw_structural_loss() trong ban GW -- moi
phan con lai (BLL, ELBO, 3-phase schedule, UMAP, landscape, metrics) deu
GIONG HET ban GW de dam bao so sanh cong bang giua 2 phuong phap.
...
Proposition 5.3:
  ...
  - Student  : ViT-Small/16 (timm, KHONG pretrained -- random init) + BLL,
               distill tu dau bang ELBO + standard KD soft-target loss
               (Hinton 2015), KHONG dung Gromov-Wasserstein.
...
Standard KD (Hinton 2015) trong khung BLL nhieu-particle: voi MOI sample
trong batch, ca teacher va student deu sample K particle tu posterior
(giong het ben GW, de UMAP/ELBO van hoat dong nhu cu). Khac biet DUY NHAT
nam o cach so sanh K particle cua teacher voi K particle cua student:
thay vi giai bai toan Gromov-Wasserstein (so khop CAU TRUC noi tai giua
cac particle, khong can chung he toa do), standard KD don gian tinh KL
divergence TRUC TIEP giua particle thu k cua teacher va particle thu k
cua student (cung temperature T), roi lay trung binh qua K va qua batch
B -- xem hinton_kd_loss(). Day la cach "matching truc tiep tung particle"
thay vi "matching cau truc hinh hoc giua cac particle".
...
Cac diem giu nguyen (logic khong doi, chi doi thanh phan distillation loss):
  ...
  - 3-phase schedule (task-only -> posterior -> distillation) cho student.
...
#     figures/
#     GWGA/
#       GWGA_single_input_vit.py
#     standard_KD.py              <-- file nay (Hinton 2015 baseline)
#
# Khac voi ban GWGA/GWGA_single_input_vit.py (nam trong thu muc con GWGA/,
# nen phai len 1 cap de toi dataset/), file nay nam TRUC TIEP o project
# root -- nen _THIS_DIR DA CHINH LA project root, khong can _PROJECT_ROOT.
...
class Config:
    ...
    kd_alpha_max: float = 1.0     # trong so toi da cua soft-target KD loss (phase 3)
    ...
    # TRUC TIEP giua particle thu k cua teacher va particle thu k cua
    # student (k = 0..K-1), roi lay trung binh qua K va batch B -- xem
    # hinton_kd_loss(). T cao hon -> phan phoi "mem" hon, lo nhieu thong
    # tin "dark knowledge" giua cac lop sai hon (dung gia tri T=4 theo de
    # xuat goc cua Hinton et al., 2015 cho cac bai toan classification
    # nhieu lop).
    ...
    base_output_dir = current_dir / "output_vit_cifar_bll_kd_standard"
...
STUDENT_KEY = "ViT-Small (student, BLL+KD)"
...
def hinton_kd_loss(teacher_resp: torch.Tensor, student_resp: torch.Tensor,
    ...
    teacher_resp, student_resp: [B, K, C] -- K particle logits cho moi
    sample trong batch (sample tu posterior BLL, GIONG HET shape dung trong
    ban GW, de moi phan UMAP/ELBO/landscape khac khong doi).

    KHAC voi Gromov-Wasserstein (von so khop CAU TRUC hinh hoc giua K
    particle ma KHONG can chung he toa do): o day ta so sanh TRUC TIEP
    particle thu k cua teacher voi particle thu k cua student (cung index
    k, cung "noise" eps ngam dinh ve vai tro), tinh KL-divergence cho tung
    cap (b, k) roi lay trung binh deu qua B*K -- dung "batchmean" cua
    F.kl_div, dung tinh than averaging giong cach `ce` duoc tinh trong
    distill_student() (reshape [B,K,C] -> [B*K, C]).
    ...
    """
    B, K, C = student_resp.shape
    T = cfg.kd_temperature
    teacher_log_probs = F.log_softmax(teacher_resp.float() / T, dim=-1)
    student_log_probs = F.log_softmax(student_resp.float() / T, dim=-1)
    teacher_probs = teacher_log_probs.exp()
    kd = F.kl_div(
        student_log_probs.reshape(B * K, C),
        teacher_probs.reshape(B * K, C),
        reduction="batchmean",
    ) * (T ** 2)
    return kd
...
def student_schedule_weights(epoch, cfg: Config):
    """3-phase schedule: (1) task-only CE, (2) + KL-to-prior (ELBO) ramp,
    (3) + soft-target KD loss ramp (gamma o day la trong so cua
    hinton_kd_loss(), thay cho gw_gamma_max o ban GW)."""
    ...
    if epoch <= phase1_end:
        ...
        return cfg.kl_beta_max, cfg.kd_alpha_max * min(1.0, frac)

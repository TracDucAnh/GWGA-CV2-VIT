"""
GWGA_random_projection.py  (ViT setup)
=======================================
Gromov-Wasserstein Alignment of Bayesian Last-Layer Posteriors
(Structural Knowledge Distillation) -- SINGLE-INPUT variant CONG THEM
**random projection (RP) augmentation** (paper Sec. 5.6 "Random projection
augmentation: an alternative null-space remedy", Eq. 6, Lemma 5.10,
Proposition 5.11 / 5.12, Corollary 5.13). Day la file phat trien TIEP tu
GWGA_single_input.py -- toan bo kien truc, du lieu, schedule, checkpoint,
UMAP, loss-landscape, ECE, Hessian-trace, OOD deu GIU NGUYEN; PHAN DUY NHAT
thay doi la cach xay dung response dung cho cost matrix GW.

Nhac lai boi canh (Proposition 5.3 trong paper): voi single-input GW
(Algorithm 1, file GWGA_single_input.py), tai MOT input co dinh h, head la
anh xa affine z = M(h) W + b(h) voi M(h) = I_C (x) h^T in R^{C x p}. Neu
p > C thi ker(M(h)) co so chieu >= p - C > 0: bat ky nhieu loan
W_i -> W_i + u voi u in ker(M(h)) deu KHONG lam thay doi cost matrix C_stud
mot chut nao -- GW single-input do do CHI la dieu kien CAN, khong phai DU,
cho viec khop hinh hoc weight-space. Batch pooling (Algorithm 2, xem
GWGA_random_projection tuong lai... nham, xem file rieng cho Algorithm 2)
thu hep khoang trong nay bang cach GOP nhieu input (Proposition 5.4), voi
chi phi Sinkhorn O((nK)^2) tang theo kich thuoc batch.

File nay dung mot phuong thuoc KHAC de xu ly cung van de, o CHI PHI CO DINH
O(K^2) KHONG phu thuoc kich thuoc batch (Sec 5.6):
  - Ve MOT LAN, truoc khi train, mot ma tran ngau nhien A in R^{r x p} voi
    cac phan tu i.i.d. N(0, 1/r) (p = out_features * in_features = kich
    thuoc VECTOR HOA cua toan bo last-layer weight W); DONG BANG A VINH
    VIEN (KHONG BAO GIO cap nhat, khong phai nn.Parameter, chi la buffer).
  - Voi moi particle W^(i) da sample tu posterior (giong het single-input),
    thay vi chi dung response z_i = M(h) W^(i) de xay cost matrix, dung
    response DA MO RONG (augmented response, Eq. 6 trong paper):
        tilde_z_i = [ M(h) W^(i) ;  A W^(i) ]  in R^{C+r}
    roi tinh pairwise distance C_stud[i,j] = d(tilde_z_i, tilde_z_j) TREN
    KHONG GIAN DA MO RONG NAY (thay vi tren z_i goc). Phan A W^(i) hoan
    toan KHONG phu thuoc input h -- no la ham cua CHINH particle W^(i) --
    nen (Remark 5.9 trong paper) no KHONG bi trung lap/huy nhau giua cac
    particle cung mot input nhu khi ghep them h truc tiep se lam.
  - Theo Lemma 5.10 + Proposition 5.11: ker(tilde M(h)) = ker(M(h)) giao
    ker(A), va VOI XAC SUAT 1 (A lien tuc) ker(A) = {0} nen tilde M(h) co
    null space RONG -- augmented cost matrix "nhin thay" TOAN BO R^p, kem
    ca phan ma GW single-input goc hoan toan mu. Proposition 5.12 (kieu
    Johnson-Lindenstrauss) cho thay ||A u||^2 xap xi ||u||^2 (sai so
    epsilon) tren ker(M(h)) khi r = O(eps^-2 log m); r CANG LON thi phan
    null-space duoc "nhin thay" cang trung thuc, nhung r ANH HUONG DEN CHI
    PHI (O(rp) moi buoc) chu KHONG anh huong den logits/CE loss (RP CHI
    duoc dung de xay cost matrix GW, KHONG bao gio duoc dua vao dau ra
    phan loai hay ham mat mat likelihood).
  - Khac voi batch pooling: RP la mot ABLATION/diagnostic (Bang so sanh
    Sec 5.6 trong paper), KHONG phai phuong phap chinh -- no bao phu toan
    bo null space MOT CACH VO DIEU KIEN (n=1, chi can r>=1), nhung mat di
    y nghia "trong khong gian response that" (A khong co y nghia kien
    truc). Muc dich cua file nay la chay ablation "RP vs batch pooling"
    tren CV-2 (Sec 6.3, muc "Random projection augmentation vs batch
    pooling") de xac dinh xem null-space coverage don thuan hay hinh hoc
    response task-specific moi la co che van chuyen thuc su.

Kien truc & du lieu (KHONG DOI so voi single-input):
  - Teacher  : ViT-Large/16 (timm, pretrained ImageNet) + Bayesian Last
               Layer (BLL).
  - Student  : ViT-Small/16 (timm, KHONG pretrained -- random init) + BLL,
               distill tu dau bang ELBO + GW(+RP) structural loss.
  - Train set       : CIFAR-100 (num_labels = 100), upsize 32x32 -> 224x224.
  - Eval (in-dist)  : CIFAR-100 test split.
  - OOD test        : CIFAR-10 (AUROC / OOD detection, KHONG dung de train).

Cac diem giu nguyen tu GWGA_single_input.py (logic khong doi):
  - 1 Config dataclass duy nhat (them cac truong rp_* cho RP).
  - Loss landscape TRUOC/SAU distill, UMAP dinh ky, 3-phase schedule
    (task-only -> posterior -> structural), ECE, Hessian-trace sharpness
    (Hutchinson), OOD AUROC (CIFAR-100 ID vs CIFAR-10 OOD).
  - Moi figure quan trong deu luu kem du lieu tho (.npz/.json) canh file
    .png, ho tro ve lai bang replot_all_from_saved() ma khong can GPU.
  - ViT dung LayerNorm (khong BatchNorm) -- khong can context-manager rieng
    cho loss landscape nhu ban CNN cu.
"""
